...
export function SourceList({ sources, className }: SourceListProps) {
  ...
  return (
    <section className={cn('chat-sources-accordion', className)}>
      <button
        type="button"
        ...
        className="chat-sources-trigger flex min-h-touch w-full items-center gap-1.5 text-left text-sm font-bold text-foreground"
      >
        ...
        <ChevronDown
          aria-hidden
          className={cn(
            'chat-sources-chevron ml-auto size-5 shrink-0 transition-transform',
            expanded && 'rotate-180',
          )}
        />
      </button>
