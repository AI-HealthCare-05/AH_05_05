...
import { V11ReportBody } from './V11ReportBody';
import { ReportMarkdown, ReportSection, reportTableClass, safeLink, slotLabels } from './reportViewPrimitives';
...
export function IntakeReportBody({ report }: { report: IntakeReport }) {
  if (report.presentationVersion === 'ai-report-v11' && report.cards) return <V11ReportBody report={report} />;
  if (report.presentationVersion === 'ai-report-v2') return <AiReportBody report={report} />;
  ...
  return <div className="min-w-0 space-y-5 text-foreground">
    ...
    {report.nutrientTotals.length > 0 && <ReportSection title="일일 성분 합계">
      ...
      <div role="region" aria-label="일일 성분 합계 표 가로 스크롤" tabIndex={0} className="overflow-x-auto focus-visible:outline-2 focus-visible:outline-primary">
        <table aria-label="일일 성분 합계" className={reportTableClass}><thead><tr><th scope="col">성분</th><th scope="col">하루 합계</th><th scope="col">포함 제품</th></tr></thead><tbody>
    ...
    {report.reportMarkdown && <ReportSection title="상세 보고서"><ReportMarkdown markdown={report.reportMarkdown} /></ReportSection>}
