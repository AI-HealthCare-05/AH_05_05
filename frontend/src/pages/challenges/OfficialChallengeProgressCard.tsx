...
function progressValue(value: number | string): number {
...
function frequencyUnit(participation: ChallengeParticipation) {
    ...
    <article aria-label={participation.challenge_name} className="flex flex-col gap-3 rounded-card bg-card p-5 shadow-card">
      ...
      {refreshRequired ? (
        <div role="status" className="flex flex-col gap-2 rounded-input bg-muted-bg p-3">
          ...
          <Button variant="secondary" onClick={onRefresh} disabled={pending} className="h-11 min-h-11">다시 불러오기</Button>
        </div>
      ) : (
        <>
          <div className="flex items-center justify-between gap-2 text-caption text-muted-foreground">
            ...
            <span className="font-bold text-primary">{progressLabel(participation.progress_rate)}% 달성</span>
          </div>
          ...
          <p className="text-caption text-foreground">{participation.completed_count} / {participation.target_count}{frequencyUnit(participation)}</p>
        </>
      )}
      ...
      {isSelfActive ? (
        <Button onClick={onCheckIn} disabled={pending || !participation.can_verify} className="h-11 min-h-11">{label}</Button>
      ) : null}
    </article>
  );
}
