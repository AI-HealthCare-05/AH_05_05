...
export function OfficialChallengeBadgePage() {
  ...
  if (!badge) return <><Header title="배지 상세" onBack={goBack} /><main className="flex flex-col gap-4 px-page-x py-5"><h1 className="text-[22px] font-bold">배지를 찾을 수 없어요</h1><button type="button" onClick={goBack} className="self-start font-bold text-primary">내 배지로 돌아가기</button></main></>;
  ...
  return (
    ...
    <main className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-page-x py-5">
      ...
      <button type="button" onClick={goBack} className="self-start font-bold text-primary">내 배지로 돌아가기</button>
    </main>
    </>
  );
}
