...
export function OfficialChallengeBadgePage() {
  ...
  if (!badge) return <><Header title="배지 상세" onBack={goBack} /><main className="flex flex-col gap-4 px-page-x py-5"><h1 className="text-[22px] font-bold">배지를 찾을 수 없어요</h1></main></>;
  ...
  return (
    ...
    <main className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-page-x py-5">
      <section className="flex flex-col gap-2 rounded-card bg-card p-5 shadow-card" aria-labelledby="badge-name">
        ...
        <p className="text-sm text-muted-foreground">{earned ? `총 ${badge.awards.length}회 획득` : '아직 획득하지 않았어요.'}</p>
      </section>
      {earned ? (
