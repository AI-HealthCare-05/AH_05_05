...
export function OfficialChallengeBadgePage() {
  ...
  const returnToBadgeList = () => navigateBackOrReplace(
    navigate,
    '/challenges/badges',
    trustedBackTarget(location.state, ['/challenges/badges']) !== null,
  );
  ...
  if (error) {
    return <><Header title="배지 상세" onBack={goBack} /><main className="flex flex-col gap-4 px-page-x py-5"><p role="alert" className="text-sm text-muted-foreground">{error}</p><Button variant="secondary" onClick={() => setReloadKey(key => key + 1)}>다시 불러오기</Button></main></>;
  }
  if (!loaded) return <><Header title="배지 상세" onBack={goBack} /><main className="px-page-x py-5"><LoadingState label="배지 상세 불러오는 중">배지를 불러오고 있어요.</LoadingState></main></>;
  if (!badge) return <><Header title="배지 상세" onBack={goBack} /><main className="flex flex-col gap-4 px-page-x py-5"><h1 className="text-[22px] font-bold">배지를 찾을 수 없어요</h1><button type="button" onClick={returnToBadgeList} className="self-start font-bold text-primary">내 배지로 돌아가기</button></main></>;
  ...
  return (
    ...
    <main className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-page-x py-5">
      ...
      <button type="button" onClick={returnToBadgeList} className="self-start font-bold text-primary">내 배지로 돌아가기</button>
    </main>
    </>
  );
}
