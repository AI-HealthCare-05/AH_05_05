...
test(`OCR 이미지 미리보기 전환과 선명한 닫기·더블클릭 닫기 (${width})`, async ({ page }, testInfo) => {
...
test('인증된 문서 OCR 계약으로 결과를 검토·수정하고 저장한다', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /파모티딘 원문/ })).toContainText('확인 필요');
