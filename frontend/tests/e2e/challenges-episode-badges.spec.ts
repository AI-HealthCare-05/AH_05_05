...
test('홈 기록은 해당 처방만 달성시키고 같은 배지를 획득 이력으로 묶는다', async ({ page }, testInfo) => {
  ...
  const otherCard = summary.getByRole('article', { name: '9월 7일 처방 복약 챌린지', exact: true });
  const other = otherCard.getByRole('link', { name: /9월 7일 처방 복약 챌린지/ });
  ...
  await expect(otherCard.getByText('완료', { exact: true })).toBeVisible();
  ...
  await expect(page.getByText('진행 중 1개 · 달성 2개', { exact: true })).toBeVisible();
