...
test('동일 이름·날짜의 처방은 실제 약으로 구분하고 같은 약·정보 누락은 지역 순번으로 구분한다', async ({ page }) => {
  ...
  await page.route('**/api/v1/med/notes/episodes', (route) => route.fulfill({
    json: variants.map((variant, index) => ({
      careEpisodeId: 7001 + index, alias: '감기약', startDate: '2026-09-01', status: 'ACTIVE', ...variant,
    })),
  }));
  ...
  await expect(selector.locator('option')).toHaveText([
    '전체',
    '감기약 · 2026년 9월 1일 · 타이레놀 외 2개 · 처방 1',
    '감기약 · 2026년 9월 1일 · 아목시실린',
    '감기약 · 2026년 9월 1일 · 타이레놀 외 2개 · 처방 2',
    '감기약 · 2026년 9월 1일 · 처방 1',
    '감기약 · 2026년 9월 1일 · 처방 2',
    `감기약 · 2026년 9월 1일 · ${longName} 외 1개`,
    '처방 · 무명약 · 처방 1',
    '처방 · 무명약 · 처방 2',
  ]);
  await selector.selectOption({ label: '감기약 · 2026년 9월 1일 · 타이레놀 외 2개 · 처방 2' });
  await expect(page).toHaveURL('/medications/notes?episodeId=7003');
  await expect(page.getByRole('heading', { name: '복약 메모 3개' })).toBeVisible();
...
test('mock 요약도 실제 약 목록의 대표약과 개수로 중복 처방을 구분한다', async ({ page }) => {
  ...
  await page.goto('/medications/notes');
  await expect(page.getByLabel('처방별 메모 필터').locator('option')).toHaveText([
    '전체', '감기약 · 2026년 9월 1일 · 아목시실린 외 1개', '감기약 · 2026년 9월 1일 · 타이레놀',
  ]);
  await page.getByLabel('처방별 메모 필터').selectOption({ label: '감기약 · 2026년 9월 1일 · 타이레놀' });
