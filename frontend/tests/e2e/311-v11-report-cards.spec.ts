...
for (const version of ['ai-report-v11', 'ai-report-v2']) {
  test(`${version} keeps report-specific basis notes only above nutrient values`, async ({ page }, testInfo) => {
    ...
    for (const [index, basisNote] of notes.entries()) {
      ...
      const valueBox = await nutrients.getByRole('img', { name: version === 'ai-report-v11' ? '비타민 D 합계 20μg, 충분 10μg' : '비타민 D 200%' }).boundingBox();
...
test('all registered nutrient totals remain visible with honest missing-reference labels', async ({ page }, testInfo) => {
  ...
  for (const width of [320, 1280]) {
    ...
    await expect(section.locator('.v11-nutrient')).toHaveCount(names.length);
    await expect(section).toContainText('비타민 A');
    const retinol = section.locator('.v11-nutrient').filter({ hasText: '레티놀' });
    ...
    await expect(retinol.getByRole('img')).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
    const columns = await section.locator('.v11-nutrient-grid').evaluate(el => getComputedStyle(el).gridTemplateColumns.split(' ').length);
...
test('v11 omits zero nutrient rows but keeps positive and unknown amounts', async ({ page }, testInfo) => {
  ...
  for (const width of [390, 1280]) {
    ...
    await page.getByRole('button', { name: '보고서 생성하기', exact: true }).click();
    const section = page.locator('#v11-nutrients');
    await expect(section.locator('.v11-nutrient')).toHaveCount(2);
...
test('v11 nutrient ranges show amounts and thresholds with upper limits anchored at eighty percent', async ({ page }, testInfo) => {
  const totals = [
    ...
    { ...report.nutrientTotals[0], nutrientName: '비타민 A', amount: '700', unit: 'μg RAE', referenceValue: '800', referenceKind: 'RNI', upperLimitValue: null, upperLimitNote: '성분 형태별 상한 기준이 달라 비교하지 않았어요.' },
  ];
  await page.route('**/api/v1/intake-reports', route => route.fulfill({ json: { ...report, nutrientTotals: totals } }));
  for (const width of [320, 390, 760, 1280]) {
    ...
    await expect(section).not.toContainText('권장 범위');
    const calcium = section.locator('.v11-nutrient').filter({ hasText: '칼슘' });
    await expect(calcium.locator('.v11-nutrient-amount')).toContainText('600');
    await expect(calcium.locator('[data-threshold="upper"]')).toContainText('3,000');
    const calciumUpper = await calcium.locator('[data-range-tick="upper"]').evaluate(el => (el as HTMLElement).style.left);
    expect(calciumUpper).toBe('80%');
    expect(await calcium.locator('[data-range-dot]').evaluate(el => Number.parseFloat((el as HTMLElement).style.left))).toBeCloseTo(16, 2);
    const vitaminD = section.locator('.v11-nutrient').filter({ hasText: '비타민 D' });
    await expect(vitaminD).toContainText('충분');
    expect(await vitaminD.locator('[data-range-dot]').evaluate(el => Number.parseFloat((el as HTMLElement).style.left))).toBeCloseTo(26.4, 2);
    const vitaminA = section.locator('.v11-nutrient').filter({ hasText: '비타민 A' });
    await expect(vitaminA.locator('[data-range-tick="upper"]')).toHaveCount(0);
    await expect(vitaminA).toContainText('성분 형태별 상한 기준이 달라');
    const iron = section.locator('.v11-nutrient').filter({ hasText: '철' });
    await expect(iron).toContainText('상한 초과');
    expect(await iron.locator('[data-range-dot]').evaluate(el => (el as HTMLElement).style.left)).toBe('100%');
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
...
test('v11 overview expands registered products responsively and closes with keyboard', async ({ page }, testInfo) => {
  for (const width of [320, 390, 1280]) {
    ...
    await expect(supplements.getByRole('listitem')).toHaveText([
      '가상 비타민 D 제품 A성분·함량 확인 필요사용자가 등록한 복용 정보 · 하루 1캡슐',
      '가상 비타민 D 제품 B성분·함량 확인 필요사용자가 등록한 복용 정보 · 하루 1정',
      '가상 함량 미확인 제품성분·함량 확인 필요사용자가 등록한 복용 정보 · 하루 1포',
    ]);
...
test('v11 displays trimmed registered doses without losing meaningful decimals', async ({ page }, testInfo) => {
  ...
  for (const width of [320, 1280]) {
    ...
    await overview.locator('summary').click();
    await expect(list.locator('.v11-product-intake')).toHaveText(doses.map(dose => `사용자가 등록한 복용 정보 · ${dose}`));
    await list.screenshot({ path: testInfo.outputPath(`trimmed-doses-${width}.png`) });
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  }
});

test('v11 keeps efficacy visible and folds remaining information independently per medicine', async ({ page }, testInfo) => {
  for (const width of [320, 1280]) {
    await page.setViewportSize({ width, height: 812 });
    await page.goto('/reports/new?source=medications');
    await page.getByRole('button', { name: '보고서 생성하기', exact: true }).click();
    ...
    await expect(second.getByText('델타 효능', { exact: true })).toBeVisible();
    ...
    await expect(first.getByText('사용자가 등록한 복용 정보', { exact: true })).toBeVisible();
    await expect(second.getByText('델타 주의', { exact: true })).toBeHidden();
...
test('v11 hides excluded product lists without hiding nutrient totals', async ({ page }, testInfo) => {
  ...
  for (const width of [320, 390, 759, 760, 1280]) {
    ...
    await page.goto('/reports/new?source=supplements');
    await page.getByRole('button', { name: '보고서 생성하기', exact: true }).click();
    const nutrient = page.locator('.v11-nutrient');
    await expect(nutrient).toBeVisible();
    await expect(nutrient.locator('details, summary')).toHaveCount(0);
    await expect(nutrient).not.toContainText('합산에서 제외');
    for (const name of names) await expect(nutrient).not.toContainText(name);
    await expect(nutrient.getByRole('img')).toBeVisible();
    await expect(page.locator('#v11-nutrients .v11-nutrient-notes')).toBeVisible();
    const columns = await page.locator('.v11-nutrient-grid').evaluate(el => getComputedStyle(el).gridTemplateColumns.split(' ').length);
...
test('v11 shows the registered two-tablet iron total and reference from the server', async ({ page }, testInfo) => {
  ...
  await page.goto('/reports/new?source=supplements');
  await page.getByRole('button', { name: '보고서 생성하기', exact: true }).click();
  const iron = page.locator('.v11-nutrient');
  await expect(iron.getByRole('heading', { name: '철', exact: true })).toBeVisible();
  await expect(iron.getByRole('img', { name: '철 합계 24mg, 권장 12mg', exact: true })).toBeVisible();
  await expect(iron.locator('.v11-nutrient-amount')).toHaveText('24mg');
  await expect(iron.locator('[data-threshold="reference"]')).toHaveText('권장12');
...
test('v11 renders server-owned cards once, keeps public guidance non-personal, and stays responsive', async ({ page }, testInfo) => {
  for (const width of [320, 390, 1280]) {
    ...
    await expect(page.locator('#v11-nutrients')).not.toContainText('가상 함량 미확인 제품');
    const iron = page.locator('.v11-nutrient').filter({ has: page.getByRole('heading', { name: '철', exact: true }) });
    ...
    await expect(page.getByRole('img', { name: '비타민 D 합계 20μg, 충분 10μg' })).toHaveCount(1);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
...
test('v11 keeps an inferred product guide notice visible below the registered medicine name', async ({ page }, testInfo) => {
  ...
  for (const width of [390, 1280]) {
    ...
    await expect(medicine.locator('h3 + .v11-hint')).toContainText('추정한 제품 안내입니다.');
