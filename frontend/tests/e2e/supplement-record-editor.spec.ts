...
test('메모와 후기 CTA는 해당 입력만 열고 저장과 삭제 시 다른 기록을 보존한다', async ({ page }) => {
  ...
  if (IS_REAL_API) expect(patches).toEqual([
    { dose_amount: 1, slots: ['MORNING'], score: null, note: '아침 식후', review_body: null },
    { dose_amount: 1, slots: ['MORNING'], score: null, note: '아침 식후', review_body: '먹기 편해요' },
    { dose_amount: 1, slots: ['MORNING'], score: null, note: null, review_body: '먹기 편해요' },
...
test('기록 저장 실패는 편집창과 draft를 유지하고 같은 payload로 재시도한다', async ({ page }) => {
  ...
  expect(patches[0]).toEqual({ dose_amount: 1, slots: ['MORNING'], score: null, note: null, review_body: '유지할 후기' });
