...
test('등록 영양제 상세는 별점 수정과 복용 정보 수정 진입만 제공하고 하단 중단 버튼은 없다', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '오메가3 제품 상세정보' })).toBeVisible();
  ...
  await expect(page.getByRole('button', { name: '복용 중단하기' })).toHaveCount(0);
  await page.getByRole('button', { name: '복용 정보 수정', exact: true }).click();
  await expect(page.getByRole('dialog', { name: '복용 정보 수정' }).getByRole('button', { name: '저장', exact: true })).toBeVisible();
});
