...
test('supplement add opens directly while selection does not delete immediately', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 320, height: 740 });
  await page.goto('/supplements');
  const listHeader = page.getByRole('heading', { name: /영양제 \d+개/ }).locator('..');
