...
5. 사용자가 복약메모·복약기록을 정리·요약하거나 진료 전에 준비하려는 목적을 직접 말하면 MEDICATION_NOTE_SUMMARY입니다. 현재 입력 자체에 그 요청이 없으면 직전 대화가 복약메모였더라도 이 intent가 아닙니다. `test`, `ㅇㅇ`처럼 뜻을 알 수 없는 낱말이거나, 마침표·물음표를 반복한 것처럼 글자 없이 기호·자모·숫자만 있는 입력은 직전 주제를 이어받지 말고 OFF_TOPIC으로 두세요. 제품명·성분명·증상이 하나라도 있으면 무의미한 입력이 아닙니다.
