...
class OpenAIRecoveryGuideGenerator:
    ...
    async def generate(
        ...
        return RecoveryGuideResult(
            care_episode_id=(
                patient_context.care_episode_id
            ),
    ...
    def _build_sources(
        ...
        for medication in patient_context.medications:
            patient_source_ids.update(
                medication.source_field_ids
            )

        for instruction in patient_context.instructions:
            if instruction.source_field_id is not None:
                patient_source_ids.add(
                    instruction.source_field_id
                )

        for schedule in (
            patient_context.follow_up_schedules
        ):
            patient_source_ids.update(
                schedule.source_field_ids
            )
