...
def build_result(
    *,
    ...
    safety_notice: str = ("이 안내는 의료진의 진료를 대체하지 않습니다."),
) -> RecoveryGuideResult:
    return RecoveryGuideResult(
        ...
        guide_content=RecoveryGuideContent(
            ...
            public_information=[],
            lifestyle_guide=(lifestyle_guide or []),
            warning_signs=[],
            follow_up_schedule=[],
...
async def test_validate_returns_safe_for_safe_guide() -> None:
    validator = RuleBasedOutputSafetyValidator()
    result = build_result(
        medication_guide=[
            "처방받은 방법대로 복용하세요.",
            "임의로 약을 중단하지 마세요.",
...
async def test_validate_blocks_medication_change_instruction() -> None:
    validator = RuleBasedOutputSafetyValidator()
    result = build_result(medication_guide=["오늘부터 아스피린 복용을 중단하세요."])
...
async def test_validate_blocks_new_diagnostic_assertion() -> None:
    validator = RuleBasedOutputSafetyValidator()
    result = build_result(patient_instructions=["현재 증상은 폐렴으로 진단됩니다."])
...
async def test_validate_blocks_treatment_decision() -> None:
    validator = RuleBasedOutputSafetyValidator()
    result = build_result(patient_instructions=["현재 상태에는 수술이 필요합니다."])
...
async def test_validate_restricts_missing_safety_notice() -> None:
    validator = RuleBasedOutputSafetyValidator()
    result = build_result(
        medication_guide=["처방받은 방법대로 복용하세요."],
...
async def test_validate_blocks_unsafe_safety_notice() -> None:
    validator = RuleBasedOutputSafetyValidator()
    result = build_result(
        safety_notice=("오늘부터 아스피린 복용을 중단하세요. 이 안내는 의료진의 진료를 대체하지 않습니다.")
    )
...
async def test_validate_blocks_patient_medication_mismatch() -> None:
    ...
    result = build_result(medication_guide=[("아스피린 · 2정 · 1일 1회 · 아침 식후 복용 · 7일")])
    validator = RuleBasedOutputSafetyValidator()
