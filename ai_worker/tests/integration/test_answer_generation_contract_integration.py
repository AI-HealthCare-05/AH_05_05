"""Chain 4 프롬프트가 실제 모델에서 초안을 어떻게 다시 쓰는지 측정한다.

단위 테스트는 초안 조립까지만 검증하므로 재작성 거동은 실모델로만 확인할 수 있다.
근거 검증 guard와 폴백까지 포함한 파이프라인 전체가 아니라 프롬프트 준수만 본다.
비결정적이므로 CI 게이트가 아니며, 기본 skip 상태로 개발 중 판단에 쓴다.
"""
