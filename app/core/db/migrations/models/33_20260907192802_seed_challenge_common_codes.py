...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        CREATE TABLE IF NOT EXISTS `_migration_33_challenge_code_seed` (
        ...
        INSERT IGNORE INTO `_migration_33_challenge_code_seed` (`entity_type`, `group_code`, `detail_code`)
        SELECT 'GROUP', seed.`group_code`, ''
        FROM (
            SELECT 'CHL_TYPE' AS `group_code`
        ...
        FROM (
            SELECT 'CHL_TYPE' AS `group_code`, 'T01' AS `detail_code`
        ...
        JOIN `common_code_groups` AS code_group
          ON code_group.`category` = 'CHL' AND code_group.`group_code` = seed.`group_code`
        ...
        FROM (
            SELECT 'CHL_TYPE' AS `group_code`, 'T01' AS `detail_code`, '걷기' AS `detail_name`, '일일 걸음 또는 걷기 실천' AS `description`, 1 AS `sort_order`
            UNION ALL SELECT 'CHL_TYPE', 'T02', '스트레칭', '일일 스트레칭 실천', 2
            UNION ALL SELECT 'CHL_TYPE', 'T03', '물마시기', '일일 수분 섭취 실천', 3
            UNION ALL SELECT 'CHL_PERIOD', 'D7', '7일', '7일 챌린지', 1
            UNION ALL SELECT 'CHL_PERIOD', 'D14', '14일', '14일 챌린지', 2
            UNION ALL SELECT 'CHL_PERIOD', 'D30', '30일', '30일 챌린지', 3
            UNION ALL SELECT 'CHK_TYPE', 'SELF', '자동 승인', '사용자 제출 즉시 승인', 1
            UNION ALL SELECT 'CHK_TYPE', 'MANUAL', '관리자 승인', '관리자가 인증을 검토해 승인', 2
            UNION ALL SELECT 'CHK_FREQ', 'DAILY', '매일', '챌린지 기간 동안 매일 인증', 1
            UNION ALL SELECT 'CHK_FREQ', 'WEEKLY_3', '주 3회', '완전한 7일 구간마다 3회 인증', 2
            UNION ALL SELECT 'CHK_FREQ', 'TOTAL_10', '총 10회', '전체 기간 동안 총 10회 인증', 3
        ) AS seed
        JOIN `common_code_groups` AS code_group
          ON code_group.`category` = 'CHL' AND code_group.`group_code` = seed.`group_code`
        LEFT JOIN `common_codes` AS existing
...
async def downgrade(db: BaseDBAsyncClient) -> str:
    return """
        ...
        JOIN `_migration_33_challenge_code_seed` AS seed
        ...
        JOIN `_migration_33_challenge_code_seed` AS seed
          ON seed.`entity_type` = 'GROUP' AND seed.`group_code` = code_group.`group_code`
        WHERE NOT EXISTS (
            SELECT 1 FROM `common_codes` AS codes WHERE codes.`group_id` = code_group.`id`
        );

        DROP TABLE IF EXISTS `_migration_33_challenge_code_seed`;"""
