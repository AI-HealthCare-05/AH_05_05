...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        ALTER TABLE `badges` CHANGE COLUMN `on_image_path` `image_path` VARCHAR(500) NOT NULL COMMENT '배지 이미지 상대 경로';
        ALTER TABLE `badges` DROP COLUMN `off_image_path`;
        """


async def downgrade(db: BaseDBAsyncClient) -> str:
    return """
        ALTER TABLE `badges` CHANGE COLUMN `image_path` `on_image_path` VARCHAR(500) NOT NULL COMMENT '활성화 배지 이미지 상대 경로';
        ALTER TABLE `badges` ADD `off_image_path` VARCHAR(500) NOT NULL COMMENT '비활성화 배지 이미지 상대 경로' DEFAULT '';
        """
