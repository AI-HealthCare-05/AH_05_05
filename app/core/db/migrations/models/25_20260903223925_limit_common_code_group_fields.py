...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        ...
        ALTER TABLE `common_codes` ADD CONSTRAINT `chk_common_codes_detail_code`
          CHECK (CHAR_LENGTH(`detail_code`) BETWEEN 1 AND 20 AND `detail_code` REGEXP '^[A-Z0-9_]+$');
        ALTER TABLE `common_code_groups` MODIFY COLUMN `group_name` VARCHAR(20) NOT NULL COMMENT '코드그룹명';
        ALTER TABLE `common_code_groups` MODIFY COLUMN `description` VARCHAR(200) COMMENT '코드그룹 설명';
        ALTER TABLE `common_code_groups` MODIFY COLUMN `category` VARCHAR(20) NOT NULL COMMENT '공통코드 대분류';
        ALTER TABLE `common_code_groups` MODIFY COLUMN `group_code` VARCHAR(20) NOT NULL COMMENT '코드그룹';
        ALTER TABLE `common_code_groups` ADD CONSTRAINT `chk_common_code_groups_category`
          CHECK (CHAR_LENGTH(`category`) BETWEEN 1 AND 20 AND `category` REGEXP '^[A-Z0-9_]+$');
        ALTER TABLE `common_code_groups` ADD CONSTRAINT `chk_common_code_groups_group_code`
          CHECK (CHAR_LENGTH(`group_code`) BETWEEN 1 AND 20 AND `group_code` REGEXP '^[A-Z0-9_]+$');"""


async def downgrade(db: BaseDBAsyncClient) -> str:
    return """
        ALTER TABLE `common_code_groups` DROP CHECK `chk_common_code_groups_group_code`;
        ALTER TABLE `common_code_groups` DROP CHECK `chk_common_code_groups_category`;
        ALTER TABLE `common_code_groups` MODIFY COLUMN `group_name` VARCHAR(100) NOT NULL COMMENT '코드그룹명';
        ALTER TABLE `common_code_groups` MODIFY COLUMN `description` VARCHAR(500) COMMENT '코드그룹 설명';
        ALTER TABLE `common_code_groups` MODIFY COLUMN `category` VARCHAR(50) NOT NULL COMMENT '공통코드 대분류';
        ALTER TABLE `common_code_groups` MODIFY COLUMN `group_code` VARCHAR(50) NOT NULL COMMENT '코드그룹';
        ALTER TABLE `common_codes` DROP CHECK `chk_common_codes_detail_code`;
        ALTER TABLE `common_codes` ADD CONSTRAINT `chk_common_codes_detail_code`
