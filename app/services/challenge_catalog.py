...
class ChallengeCatalogService:
    async def list(self, user_id: int, offset: int, limit: int) -> tuple[list[ChallengeCatalogResponse], int]:
        ...
        owned = await UserChallenge.filter(user_id=user_id, challenge_id__in=[row.id for row in rows]).order_by("id")
        participations = {item.challenge_id: item for item in owned}
        now = datetime.now(config.TIMEZONE)
        return [self.response(row, participations.get(row.id), now) for row in rows], total
    ...
    async def get(self, user_id: int, challenge_id: int) -> ChallengeCatalogResponse:
        ...
        owned = await UserChallenge.filter(user_id=user_id, challenge_id=challenge_id).order_by("-id").first()
        return self.response(row, owned, datetime.now(config.TIMEZONE))

    @staticmethod
    def response(challenge: Challenge, participation: UserChallenge | None, now: datetime) -> ChallengeCatalogResponse:
        ...
        return ChallengeCatalogResponse(
            ...
            can_join=bool(
                supported
                and (participation is None or participation.status == ChallengeParticipationStatus.CANCELLED)
