...
class TestAdminDetailAPI:
    ...
    async def test_allows_null_created_by_for_first_super_admin(self) -> None:
        ...
        assert response.json()["createdByAccountId"] is None
...
class TestUserDetailAPI:
    async def test_returns_user_detail(self) -> None:
        ...
        assert isinstance(body["activeAlarmCount"], int)

    async def test_returns_consents_per_item(self) -> None:
        response = await get(f"{ADMIN_USERS_URL}/9201", headers=auth_header())

        consents = response.json()["consents"]
        assert {consent["consentType"] for consent in consents} == {
            "MEDICAL_DATA",
            "AI_USAGE",
            "NOTIFICATION",
        }

    async def test_derives_withdrawn_at_from_latest_disagreement(self) -> None:
        """철회는 agreed=false 인 새 행으로 남으므로 withdrawnAt 이 채워져야 한다(ERD v3)."""
        response = await get(f"{ADMIN_USERS_URL}/9201", headers=auth_header())

        consents = {consent["consentType"]: consent for consent in response.json()["consents"]}
        assert consents["MEDICAL_DATA"]["agreed"] is True
        assert consents["MEDICAL_DATA"]["withdrawnAt"] is None
        assert consents["NOTIFICATION"]["agreed"] is False
        assert consents["NOTIFICATION"]["withdrawnAt"] is not None
