...
## 실험 8: 체인별 프롬프트 요소와 제한적 DSP 명시

### 문제

런타임 프롬프트가 Role, Task, Content, Format, Constraint와 Example을 사용하고 있었지만 사람이 읽는 문서에서 요소 경계가 명확하지 않았고, 검색 방향과 근거 판정에 적용할 DSP 범위가 구분되지 않았다.

### 실험

- RED: 네 개 주 체인에 여섯 개 표준 요소가 명시되고 Directional Query와 Evidence Reasoning에만 일곱 번째 DSP 요소가 존재하는 계약 테스트를 추가했으며 4개 사례가 실패했다.
- 수정: 네 개 주 체인에 제목과 구분선을 추가하고 여섯 개 요소를 대괄호 레이블로 통일했다.
- 수정: Directional Query에는 허용된 대상·검색어로 최대 3개 검색 방향만 만들도록 DSP를 추가했다.
- 수정: Evidence Reasoning에는 직접 근거 있음·없음·조건별 충돌을 중립적으로 비교하도록 DSP를 추가했다.
- 유지: Conversation Gate와 Answer Generation에는 DSP를 넣지 않았고 기존 feature flag도 변경하지 않았다.

### 결과

- 성공: 신규 계약 테스트 4개와 관련 회귀 테스트 64개가 통과했다.
- 성공: 전체 테스트의 1,387개 사례가 통과하고 1개가 건너뛰어졌다.
- 환경 이슈: 전체 테스트 결과 출력 후 macOS 네이티브 런타임 종료 단계에서 `recursive_mutex lock failed`가 발생해 프로세스 종료 코드는 134였다. 테스트 assertion 실패는 없었으며 Ruff와 diff 검사는 종료 코드 0으로 통과했다.
- 성능 판단: 기존 LLM 호출 내부의 지시문만 늘어나 별도 API 왕복은 없지만, 검색 자극이 늘면 후보 정밀도와 P95가 나빠질 수 있다. 최대 3개 제한과 서버 후보 검증을 유지하고 실제 활성화 전 고정 평가 세트로 비교한다.
