...
기존 공개 비교 서버의 main 원본/메모 16px 후보는 이관 전과 동일한 빌드로 유지한다. 이번 공통 Input·챗봇 추가 변경은 로컬 브랜치에서 검증했으며 공개 비교 서버에는 반영하지 않았다. 합성 데이터 전용이며 운영 데이터와 연결하지 않는다. PR·병합·운영 배포는 하지 않는다.
