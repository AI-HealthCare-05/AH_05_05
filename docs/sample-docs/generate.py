...
def render(blocks, page_note=None, w=W, h=H, margin=M):
    ...
    for kind, text in blocks:
        if kind == 'hospital':
            d.text((x, y), text, font=f(21), fill=(70, 70, 70)); y += 30
        elif kind == 'h1':
            d.text((x, y), text, font=f(40, True), fill=(15, 15, 15)); y += 56
            d.line([(x, y), (x + maxw, y)], fill=(40, 40, 40), width=3); y += 22
        elif kind == 'meta':
            d.text((x, y), text, font=f(20), fill=(45, 45, 45)); y += 29
        elif kind == 'rule':
            y += 6; d.line([(x, y), (x + maxw, y)], fill=(200, 200, 200), width=1); y += 16
        elif kind == 'section':
            y += 12
            d.text((x, y), text, font=f(24, True), fill=(20, 20, 20)); y += 38
        elif kind == 'body':
            for ln in wrap(d, text, f(20), maxw):
                d.text((x, y), ln, font=f(20), fill=(30, 30, 30)); y += 30
        elif kind == 'bullet':
            for i, ln in enumerate(wrap(d, text, f(20), maxw - 26)):
                d.text((x + (6 if i == 0 else 26), y), ('- ' if i == 0 else '') + ln,
                       font=f(20), fill=(30, 30, 30)); y += 30
        elif kind == 'mono':
            d.text((x + 6, y), text, font=f(20), fill=(25, 25, 25)); y += 29
        elif kind == 'spacer':
            y += int(text)
    # 직인
...
def render_bag(blocks):
    ...
    for kind, text in blocks:
        if kind == 'h1':
            d.text((x, y), text, font=f(34, True), fill=(20, 20, 20)); y += 50
        elif kind == 'meta':
            d.text((x, y), text, font=f(19), fill=(50, 50, 50)); y += 27
        elif kind == 'rule':
            y += 8; d.line([(x, y), (x + maxw, y)], fill=(190, 190, 180), width=2); y += 16
        elif kind == 'timebox':
            d.rectangle([x, y, x + maxw, y + 40], fill=(238, 236, 222), outline=(170, 170, 155), width=2)
            d.text((x + 12, y + 8), text, font=f(22, True), fill=(30, 30, 30)); y += 52
        elif kind == 'mono':
            d.text((x + 14, y), text, font=f(20), fill=(25, 25, 25)); y += 30
        elif kind == 'body':
            for ln in wrap(d, text, f(19), maxw):
                d.text((x, y), ln, font=f(19), fill=(45, 45, 45)); y += 27
        elif kind == 'spacer':
            y += int(text)
    return img
...
def degrade(img, seed=0, crop_side='right'):
    ...
    def coeffs(pa, pb):
        ...
        A = np.matrix(mat, dtype=float)
        B = np.array(pa).reshape(8)
        return np.array(np.dot(np.linalg.inv(A.T * A) * A.T, B)).reshape(8)
    im = im.transform((w, h), Image.PERSPECTIVE, coeffs(src, dst),
                      Image.BICUBIC, fillcolor=(246, 246, 244))

    # 2) 약간 회전
    im = im.rotate(rnd.uniform(-2.4, -1.1), resample=Image.BICUBIC,
                   expand=False, fillcolor=(246, 246, 244))
...
P1_DISCHARGE = [
    ...
    ('section', '응급 시 연락'),
    ('body', '38℃ 이상 발열, 수술창 발적·삼출, 종아리 부종·통증, 갑작스러운 호흡곤란 시 '
