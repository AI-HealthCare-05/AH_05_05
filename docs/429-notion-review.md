...
- 후속 수정 `f10ae0a` 뒤 PR #448 병합 main `181f32818cf092a7e7fb89a53451045b172c918f`를 충돌 없이 통합했다. 최신 공통 탭과 조합한 목록/완료 처방 기본 일시 핵심 2건은 passed/실패0이며, 별도 최종 `tsc --noEmit` exit0을 확인했다. 이번 통합에서 backend 제품 변경은 없어 직전 SQLite 4건은 재실행하지 않았다. 최신 main 기준 캡처는 notes-375/390/1280 및 first-dose-COMPLETED이며 나머지 캡처는 직전 후속 검증 시점이다.
