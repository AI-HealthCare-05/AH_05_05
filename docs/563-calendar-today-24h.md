...
- 로컬 프런트엔드 변경이다. API·DB·복용 정책 변경, PR·배포·운영 데이터 조작은 하지 않았다.
...
- 후속 통합 주의: #556의 일반 입력 16px 변경과 같은 Input 파일을 다룬다. #556이 반영된 대상 브랜치에 병합할 때 해당 16px 변경을 새 BaseInput에도 유지해야 한다. 이 브랜치는 main에서 독립 분기하므로 #556이나 #562를 임의로 섞지 않았다.
