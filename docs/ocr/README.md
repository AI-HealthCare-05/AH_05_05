...
전처리 색상·선명도 v3.2 실험과 실측 결과는
[OCR v3.2 색상·선명도 전처리 실험](./ocr-v3.2-color-sharpness-matrix.md)에 기록했다.
GrabCut 경량화 v3.1의 전처리·OCR·LLM 비교는
[OCR v3.1 변형 매트릭스](./ocr-v3.1-variant-matrix.md)에 기록했다.

코드와 `.env.example`의 기본 전처리는 `v3.4.1`이다. GrabCut 1회의 기존 기하 보정을 유지하면서
승인된 원근 보정 경로에만 약한 색상·선명화를 적용한다. 원본 16장에서는 239/309→250/309로 개선되어
승격 기준을 통과했다. [정확도·시간·강건성 및 남은 실패](./ocr-v34-adaptive-20260907.md)를 함께 확인한다.
이 작업에서 실제 `.env`나 실행 중인 서비스는 변경하지 않았다. 환경에 명시된 버전이 있으면 코드 기본값보다
우선하며, 실제 설정 전환은 별도 작업으로 `.env` 변경과 `ocr-worker` 재시작이 필요하다.

문자 줄 기울기를 영상만으로 추정하는 선택 후보 `v3.3.1`도 지원한다.
원근·조명 보정 뒤 확신이 있는 경우에만 회전하며 OCR 요청을 추가하지 않는다.
이 후보는 기본값이 아니다. 실험할 때만 `OCR_PREPROCESS_VERSION=v3.3.1`로
선택한다. 작은 글자·점선·각도가 불일치하는 문서는 보정을 건너뛸 수 있다.
검증 결과는 [문자 기울기 보정 실험](./ocr-v3.3-text-deskew.md)을 참고한다.

## 전체 흐름

906 통합본은 최신 UI의 별칭·등록 재편집 흐름과 병원명 OCR을 함께 지원한다.
별칭은 병원명으로 초기화하고 사용자가 수정한 값은 유지한다(별칭 최대 50자).
병원명 저장에는 `31_20260906000000_add_care_episode_hospital_name.py` 마이그레이션이 필요하다.
903의 이전 23번 병원명 마이그레이션을 이미 적용한 DB는 컬럼·마이그레이션 이력을 확인한 뒤 전환해야 한다.
파일 통합 작업에서는 실행 중 DB나 컨테이너에 마이그레이션을 적용하지 않았다.
...
7. 약품명을 누르면 약 정보를 수정할 수 있다.
   - 약 추가·수정·삭제 가능
   - 병원명, 조제일, 약품명, 함량, 1회 투약량, 횟수, 일수 확인
8. **저장**을 누른다.
