...
- 원본·전처리 이미지는 전용 메모리 Redis에만 임시 보관하며 디스크 파일로 저장하지 않는다.
- 미리보기는 소유자의 만료되지 않은 `READY_FOR_REVIEW`에서만 제공한다.
- 확정·취소·최종 실패 또는 TTL 만료 시 이미지를 삭제한다. `COMPLETE` 상태의 두 이미지 API는 404다.
- 구조화 결과·확정된 복약 정보는 보존하며, 완료 후 등록 수정은 사진 없이 텍스트 정보로 진행한다.
- 메모리 저장소 재시작으로 이미지가 사라져도 검토 결과 정보는 남는다. [운영·레거시 정리](./ocr/ocr-memory-only-storage-20260913.md)를 참고한다.
